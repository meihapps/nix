{ pkgs, ... }:
{
  ...
  programs.fish = {
    ...
    shellInit = ''
      set -gx EDITOR helix
      set -gx PATH $PATH /home/meihapps/.lmstudio/bin
    '';
    loginShellInit = ''
    ...
    functions = {
      ...
      vpn = ''
        switch $argv[1]
            case up
                sudo wg-quick up us-sjc-wg-302
            case down
                sudo wg-quick down us-sjc-wg-302
            case status
                if ip link show us-sjc-wg-302 &>/dev/null
                    echo up
                else
                    echo down
                end
